    ...
    <fieldset>
      ...
      <div class="mt-4 grid grid-cols-1 gap-y-6 sm:grid-cols-2 sm:gap-x-4">
        <label 
          ...
          :class="{'outline-2 outline-primary-700': method.type === checkoutStore.deliveryMethod.type}" 
          class="relative flex cursor-pointer rounded-lg border-none -outline-offset-2 bg-white p-4 focus:outline-none"
        >
          ...
          <span class="flex flex-1">
            <span class="flex flex-col">
              <span class="block text-sm font-medium text-gray-900">{{ method.type }}</span>
          ...
          <svg v-if="method.type === checkoutStore.deliveryMethod.type" class="size-5 text-indigo-600" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true" data-slot="icon">
